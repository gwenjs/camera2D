...
describe('createCamera2DHandle', () => {
  ...
  it('follow() adds FollowTarget and clears active camera paths', () => {
    ...
    handle.follow(targetId, { lerp: 0.25, offset: { x: 8, y: -3 } })
  ...
  it('setZoom(), setBounds(), shake(), activate(), and deactivate() mutate camera state', () => {
    const engine = createMockEngine()
    seedCamera(engine, cameraId)
    const handle = createCamera2DHandle(cameraId, engine as never, log as never)
    ...
    expect(engine.getComponent(cameraId, CameraShake)).toMatchObject({ trauma: 1 })
  })
